...
##
# import adafruit_ahtx0  # Commented out because we're using MCP9808
...
class TempMachine(StateMachine):
    ...
    # My Model Temp Sensor
    temp_sensor = adafruit_mcp9808.MCP9808(i2c)  # Changed thSensor -> temp_sensor
    ...
    def __init__(self):
        ...
        # Initialize LED states based on the initial state (Celsius)
        self.blueLight.on()
        self.redLight.off()
    ...
    def on_enter_Celsius(self):
        ...
        self.blueLight.on()  # Turn on blue LED for Celsius
        self.redLight.off()  # Ensure red LED is off
        if DEBUG:
    ...
    def on_enter_Fahrenheit(self):
        ...
        self.redLight.on()  # Turn on red LED for Fahrenheit
        self.blueLight.off()  # Ensure blue LED is off
        if DEBUG:
    ...
    def getFahrenheit(self):
        t = self.temp_sensor.temperature  # Changed thSensor -> temp_sensor
        return (((9 / 5) * t) + 32)
    ...
    ##
    def getCelsius(self):
        return self.temp_sensor.temperature  # Changed thSensor -> temp_sensor
        ...
        while not self.endDisplay:
            try:
                ...
                if self.activeScale == 'C':
                    ...
                    line2 = f"T:{temp:0.1f}C"
                else:
                    temp = self.getFahrenheit()
                    line2 = f"T:{temp:0.1f}F"

                self.screen.updateScreen(line1 + line2)
                sleep(1)
            except Exception as e:
                print(f"Error in displayTemp: {e}")
    ...
    def cleanup(self):
        ...
        self.blueLight.off()
        self.endDisplay = True
        sleep(1)  # Give time for the display thread to finish
...
while repeat:
    try:
        ...
        tempMachine.cleanup()  # Call the cleanup method
